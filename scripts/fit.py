...
from graspqp.core.initializations import initialize_convex_hull, initialize_tabletop_grasp
...
parser.add_argument("--initialization", default="convex_hull", type=str, choices=["random", "convex_hull", "tabletop"])
...
if args.initialization in ("convex_hull", "tabletop"):
    _initialize(hand_model, object_model, args)
...
for step in tqdm(range(1, args.n_iter + 1), desc="optimizing"):
    ...
    if args.reset_epochs is not None and step % args.reset_epochs == 0 and (step < args.n_iter - 2 * args.reset_epochs):
        ...
        if reset_mask.sum() > 0:
            ...
            _initialize(hand_model, object_model, args, env_mask=reset_mask)
    ...
    if step == 1 or step % 500 == 0:
        # Step 1 is the seed: the pose the initializer produced, before the
        # optimiser has moved it. Without it an initializer change cannot be
        # told from what the optimisation does with it.
